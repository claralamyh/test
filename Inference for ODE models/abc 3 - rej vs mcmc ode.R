...
par(mfrow=c(1,2))
R0 <- out[,1]/out[,2]*261
hist(R0, xlab="R_0", main="ABC rejection", xlim=c(1.2,2))
abline(v=mean(R0), lwd=1.3) #1.582832
abline(v=quantile(R0, c(0.025,0.975)), 
       col="blue", lty="dashed", lwd=1.3) #1.341943 1.877758  

R0 <- out2[,1]/out2[,2]*261
hist(R0, xlab="R_0", main="ABC-MCMC", xlim=c(1.2,2))
abline(v=mean(R0), lwd=1.3) #1.626474
abline(v=quantile(R0, c(0.025,0.975)), 
       col="blue", lty="dashed", lwd=1.3) #1.362861 1.885687 
...
plot(ts(I.mean, start=0, deltat=dt), ylim=c(0,45),
