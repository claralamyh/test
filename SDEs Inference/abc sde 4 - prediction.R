...
abline(v=mean(R0), lwd=1.3) # 1.510991
abline(v=quantile(R0, c(0.025,0.975)), 
       col="blue", lty="dashed", lwd=1.3) #0.868674 2.096484 
...
pred2 <- array(0, dim=c(endT/dt+1, nrow(out2), 2))
for(i in 1:nrow(out2)) {
  sim <- euler.m(c(254,7), out2[i,], endT, dt)
  pred2[,i,1] <- sim[,1]
  pred2[,i,2] <- sim[,2]
}
...
abline(v=mean(R0), lwd=1.3) # 1.80686
abline(v=quantile(R0, c(0.025,0.975)), 
       col="blue", lty="dashed", lwd=1.3) #1.263261 2.389629 
